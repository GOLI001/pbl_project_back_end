...
def create_app():
    ...
    api = Api(
        ...
        description='API для контроля экранного времени',
        doc='/api/docs',
        prefix='/api',
        authorizations={
            'Bearer': {
    ...
    class Profile(Resource):
        ...
        def get(self):
            ...
            def get_profile():
                ...
                if user:
                    return {'user': {'id': user['id'], 'email': user['email'], 'name': user['name'], 'level': user['level'], 'streak': user['streak']}}, 200
...
# ВАЖНО: Создаем глобальную переменную app
app = create_app()
